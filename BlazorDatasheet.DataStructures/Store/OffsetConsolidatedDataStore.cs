...
/// <summary>
/// Consolidated data store that applies row/column offsets instead of shifting data when rows or columns are inserted.
/// </summary>
public class OffsetConsolidatedDataStore<T> : ConsolidatedDataStore<T> where T : IEquatable<T>
{
    private readonly OffsetManager _rowOffsets = new();
    private readonly OffsetManager _colOffsets = new();

    public OffsetConsolidatedDataStore(int minArea = 0, bool expandWhenInsertAfter = true) : base(minArea, expandWhenInsertAfter)
    {
    }
